...
void Md::sendPwm(uint16_t address_, bool phase_, uint16_t power_){
	...
	send_buf[2] = (uint8_t)(PWM << 1) | (uint8_t)phase_;
	send_buf[3] = (uint8_t)((power_ >> 8) & 0xff);
	send_buf[4] = (uint8_t)(power_ & 0xff);
...
void Md::sendSpeed(uint16_t address_, bool phase_, uint16_t speed_, uint16_t end_){
	...
	send_buf[0] = 0x01;
	send_buf[1] = (uint8_t)address_;
	send_buf[2] = (uint8_t)(SPEED << 1) | (uint8_t)phase_;
	send_buf[3] = (uint8_t)((speed_ >> 8) & 0xff);
	send_buf[4] = (uint8_t)(speed_ & 0xff);
	send_buf[5] = (uint8_t)((end_ >> 8) & 0xff);
	send_buf[6] = (uint8_t)(end_ & 0xff);
