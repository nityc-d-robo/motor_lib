	...
	while(true){
		...
		if(return_status != RX_SIZE){
			for(auto itr=finish_queue.begin(); itr!=finish_queue.end(); itr++){
				if((*itr)[Header::MODE] == FinishStatus::STATUS and (*itr)[Header::ADDRESS] == (address_ | IdType::SD) and (*itr)[Header::SEMI_ID] == IdType::MASTER){
		...
		if(read_buf[Header::ADDRESS] != (address_ | IdType::SD) or read_buf[Header::SEMI_ID] != IdType::MASTER or read_buf[Header::MODE] != FinishStatus::STATUS){
...
int MotorLib::Sd::sendStatus(uint8_t address_, uint8_t semi_id_, SdStatus& sd_status_, uint32_t usb_timeout_){
	...
	while(true){
		...
		if(return_status != RX_SIZE){
			for(auto itr=finish_queue.begin(); itr!=finish_queue.end(); itr++){
				if((*itr)[Header::MODE] == FinishStatus::STATUS and (*itr)[Header::ADDRESS] == (address_ | IdType::SD) and (*itr)[Header::SEMI_ID] == (semi_id_ | IdType::SM)){
